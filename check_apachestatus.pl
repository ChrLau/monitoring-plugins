#!/usr/bin/perl -w
##
## Modified by Christian Lauf, 21.04.2016
## Changed hardcoded URI /server-status to parameter (-u, --uri=)
## https://github.com/ChrLau/monitoring-plugins/check_apachestatus.pl
## [x] Patch sent upstream
##
...
#  $request = HTTP::Request->new(GET => $o_proto.$o_host.'/server-status');
  $request = HTTP::Request->new(GET => $o_proto.$o_host.'/-system/status');
} else {
#  $request = HTTP::Request->new(GET => $o_proto.$o_host.':'.$o_port.'/server-status');
  $request = HTTP::Request->new(GET => $o_proto.$o_host.':'.$o_port.'/-system/status');
}
